...
from LuciferMoringstar_Robot import temp, SUPPORT, PICS, ADMINS, CREATOR_USERNAME, CREATOR_NAME, AUTH_CHANNEL, CUSTOM_FILE_CAPTION, SAVE_FILES, START_MESSAGE, LOG_CHANNEL
...
from LuciferMoringstar_Robot.translation import STATUS_MESSAGE
from database.core import parser, split_quotes
...
async def start(bot: lucifermoringstar_robot, update):
    ...
    if not await db.is_user_exist(update.from_user.id):
        ...
        await bot.send_message(LOG_CHANNEL, text=f"New User \n\nID » {update.from_user.id}\nName » {update.from_user.mention}\nUsername » {update.from_user.username} \n\nBy » {temp.Bot_Username}")
    ...
    if len(update.command) ==2 and update.command[1] in ["subscribe"]:
        FORCES = ["https://telegra.ph/file/b2acb2586995d0e107760.jpg"]
        invite_link = await bot.create_chat_invite_link(int(AUTH_CHANNEL))
        pr0fess0r_99 = [[ InlineKeyboardButton("🚀 JOIN OUR CHANNEL 🚀", url=invite_link.invite_link) ]]
        pr0fess0r_99 = InlineKeyboardMarkup(pr0fess0r_99)
        await update.reply_text(text = f"""<i><b>𝙷𝙴𝙻𝙻𝙾 {update.from_user.mention}. \n 𝚈𝙾𝚄 𝙷𝙰𝚅𝙴 <a href="{invite_link.invite_link}"> 𝙽𝙾𝚃 𝚂𝚄𝙱𝚂𝙲𝚁𝙸𝙱𝙴𝙳</a> 𝚃𝙾 <a href="{invite_link.invite_link}">𝙼𝚈 𝚄𝙿𝙳𝙰𝚃𝙴 𝙲𝙷𝙰𝙽𝙽𝙴𝙻</a>.𝚂𝙾 𝚈𝙾𝚄 𝙳𝙾 𝙽𝙾𝚃 𝙶𝙴𝚃 𝚃𝙷𝙴 𝙵𝙸𝙻𝙴𝚂 𝙾𝙽 𝙱𝙾𝚃 𝙿𝙼, 𝚅𝙸𝙰 𝙰𝙽𝙳 𝙶𝚁𝙾𝚄𝙿 (𝙵𝙸𝙻𝚃𝙴𝚁)</i></b>""", reply_markup=pr0fess0r_99)
        return

    if len(update.command) != 2:
        btn = [[
           InlineKeyboardButton("🍁 𝙰𝙳𝙳 𝙼𝙴 𝚃𝙾 𝚈𝙾𝚄𝚁 𝙶𝚁𝙾𝚄𝙿 🍁", url=f"http://t.me/{temp.Bot_Username}?startgroup=true")
           ],[
        ...
        await bot.send_photo(photo=random.choice(PICS), chat_id=update.chat.id, caption=START_MESSAGE.format(mention=update.from_user.mention, name=temp.Bot_Name, username=temp.Bot_Username), reply_markup=InlineKeyboardMarkup(btn))
...
async def about(bot: lucifermoringstar_robot, update):
    ...
    await bot.send_photo(photo=random.choice(PICS), chat_id=update.chat.id, caption=ABOUT_MESSAGE.format(name = CREATOR_NAME, username = CREATOR_USERNAME, py3_version = temp.PY3_VERSION, pyro_version = temp.PYRO_VERSION, version = temp.BOT_VERSION, bot = temp.Bot_Name, link = temp.Bot_Username, source = "https://github.com/PR0FESS0R-99/LuciferMoringstar-Robot"), reply_markup=InlineKeyboardMarkup(pr0fess0r_99))
...
@lucifermoringstar_robot.on_message((filters.private | filters.group) & filters.command('settings'), group=6)
...
@lucifermoringstar_robot.on_message((filters.private | filters.group) & filters.command('set_temp'), group=7)
async def save_template(client, message):
    sts = await message.reply_text("⏳️")
    await asyncio.sleep(0.3)
    userid = message.from_user.id if message.from_user else None
    if not userid:
        return await message.reply_text(f"𝚈𝙾𝚄𝚁 𝙰𝚁𝙴 𝙰𝙽𝙾𝙽𝚈𝙼𝙾𝚄𝚂 𝙰𝙳𝙼𝙸𝙽. /connect {update.chat.id} 𝙸𝙽 𝙿𝙼")
...
@lucifermoringstar_robot.on_message((filters.private | filters.group) & filters.command('setwelcome'), group=8)
...
@lucifermoringstar_robot.on_message((filters.private | filters.group) & filters.command('setspell'), group=9)
...
@lucifermoringstar_robot.on_message((filters.private | filters.group) & filters.command('setcaption'), group=10)
